...
export default function Navbar() {
  ...
  return (
    <nav className="fixed top-0 w-full z-50 bg-white border-b border-gray-100 shadow-sm">
        ...
        <div className="flex items-center space-x-4">
          <div className="hidden sm:flex items-center space-x-2">
            <Link href="/contact">
              <Button
                variant="outline"
                className="border-2 font-bold uppercase text-xs tracking-widest h-11 px-6"
              >
                Contact
              </Button>
            </Link>
      ...
      <div
        className={`fixed inset-0 bg-slate-950/40 backdrop-blur-sm z-50 lg:hidden transition-opacity duration-300 ${isMobileMenuOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"}`}
          ...
          <div className="mt-auto pt-10 border-t border-slate-100">
            <div className="flex flex-col gap-4">
              <Link href="/contact" onClick={() => setIsMobileMenuOpen(false)}>
                <Button
                  variant="outline"
                  className="w-full border-2 font-bold uppercase text-xs tracking-widest h-14"
                >
                  Contact Us
                </Button>
              </Link>
